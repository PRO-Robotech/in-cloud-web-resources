...
- children:
    ...
    - key: instances
      label: Instances
      link: /{{ $.Values.basePrefix }}/{cluster}/api-table/{{ $instRes.apiGroup }}/{{ $instRes.apiVersion }}/{{ $instRes.resourceName }}
    {{ end }}
  key: home
  label: Home
  {{ end }}
...
- children:
    ...
    - key: argocd-application
      label: Applications
      link: /{{ $.Values.basePrefix }}/{cluster}/api-table/argoproj.io/v1alpha1/applications
    {{ end }}
    {{ if .items.applicationset }}
    - key: argocd-applicationset
      label: ApplicationSets
      link: /{{ $.Values.basePrefix }}/{cluster}/api-table/argoproj.io/v1alpha1/applicationsets
    {{ end }}
    {{ if .items.appprojects }}
    - key: argocd-appprojects
      label: Projects
      link: /{{ $.Values.basePrefix }}/{cluster}/api-table/argoproj.io/v1alpha1/appprojects
    {{ end }}
    {{ with .extraItems }}
      {{ . | toYaml | nindent 4 }}
    {{ end }}
...
- key: hbf
  ...
  children:
    ...
    - key: hbf-hosts
      label: Hosts
      link: "/{{ $.Values.basePrefix }}/{cluster}/api-table/netguard.sgroups.io/{{ $.Values.addons.hbf.apiVersion }}/hosts?resources=/v1/namespaces"
    {{ end }}
    {{ if .items.networks }}
    - key: hbf-networks
      label: Networks
      link: "/{{ $.Values.basePrefix }}/{cluster}/api-table/netguard.sgroups.io/{{ $.Values.addons.hbf.apiVersion }}/networks?resources=/v1/namespaces"
    {{ end }}
    ...
    - key: hbf-svcfqdnrules
      label: SvcFqdnRules
      link: "/{{ $.Values.basePrefix }}/{cluster}/api-table/netguard.sgroups.io/{{ $.Values.addons.hbf.apiVersion }}/svcfqdnrules?resources=/v1/namespaces"
    {{ end }}
    {{ with .extraItems }}
      {{ . | toYaml | nindent 4 }}
    {{ end }}
